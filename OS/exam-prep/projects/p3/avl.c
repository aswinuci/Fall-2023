...
static struct node *
find_min_node(struct node *root) {
...
static struct node *
delete_node(struct avl *avl, struct node *root, const char *item) {
    if (root == NULL) {
        return root;
    }
...
int avl_delete(struct avl *avl, const char *item) {
    if (avl == NULL || avl->state == NULL || avl->state->root == NULL || item == NULL) {
        return -1;
    }

    avl->state->root = delete_node(avl, avl->state->root, item);
    return 0;
}
