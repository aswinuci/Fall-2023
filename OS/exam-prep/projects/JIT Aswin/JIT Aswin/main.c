...
generate(const struct parser_dag *dag, FILE *file)
{
	...
	fprintf(file, "double (*sigmoid)(double) = (double (*)(double))%ld;\n", (long) sigmoid);
    fprintf(file, "void evaluate(void) {\n");
	reflect(dag, file);
	fprintf(file, "double result = sigmoid(t%d);\n", dag->id);
	fprintf(file, "printf(\"The Value of the expression is %%f and the Sigmoid is %%f.\", t%d, result);\n}", dag->id);
}

typedef void (*evaluate_t)(void);
